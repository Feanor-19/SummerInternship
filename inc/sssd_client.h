...
//! @brief Converts given username to SID.
...
//! @return `true` if everything is okay, `false` if some error occurs. 
//! @note Error text can be also obtained with strerror(error code) (see string.h)
bool username_to_sid(const char *username, char **SID_p, int* error_code_p = NULL, const char **error_text_p = NULL);
...
//! @brief Checks if given username is found in the domain.
//! @attention If `true` is returned, given UID IS found in the domain.
//!            If `false` is returned AND *error_code_p == 0, given UID IS NOT found.
//!            If `false` is returned AND *error_code_p != 0, some error occured.
//! @param[in]  username    Username;
...
//! @return `true` if everything is okay, `false` if some error occurs. 
//! @note Error text can be also obtained with strerror(error code) (see string.h)
bool is_domain_username(const char *username, int* error_code_p = NULL, const char **error_text_p = NULL);
...
//! @brief Returns list of DomainGroup structs for user by their SID.
//! @param[in] user_SID User's SID.
//! @param[out] groups_p The result of the function. Must be freed using `free_DomainGroups()`.
//! @param[out] error_code  Error code (optional, changed only if `false` is returned);
//! @return `true` if everything is okay, `false` if some error occurs. 
//! @note Error text can be obtained with strerror(error code) (see string.h)
//! @note error_code == ENOENT, if no domain groups are found.
bool get_domain_groups_by_user_sid(const char *user_SID, DomainGroups *groups_p, int *error_code_p = NULL);